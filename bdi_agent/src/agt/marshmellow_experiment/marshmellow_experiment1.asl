...
/* Piano principale */
+!go_towards(pot(N)) : player(StartX, StartY,_,_,_) <-
    !bfs_search(StartX, StartY, pot(N), Path);
    !follow_path(Path);
    !face_object(pot(N)).
...
+!find_adjacent_position(TargetX, TargetY, AdjX, AdjY) : width(W) & height(H) <-
    ...
    AdjX1 = TargetX + 1;
    AdjY1 = TargetY;
    if (AdjX1 >= 0 & AdjX1 < W & AdjY1 >= 0 & AdjY1 < H & not cell(_, AdjX1, AdjY1) & not object(_, AdjX1, AdjY1) & dist(AdjX1, AdjY1, _)) {
        AdjX = AdjX1;
        AdjY = AdjY1;
    } else {
        // Try west
        AdjX2 = TargetX - 1;
        AdjY2 = TargetY;
        if (AdjX2 >= 0 & AdjX2 < W & AdjY2 >= 0 & AdjY2 < H & not cell(_, AdjX2, AdjY2) & not object(_, AdjX2, AdjY2) & dist(AdjX2, AdjY2, _)) {
            AdjX = AdjX2;
            AdjY = AdjY2;
        } else {
            // Try south
            AdjX3 = TargetX;
            AdjY3 = TargetY + 1;
            if (AdjX3 >= 0 & AdjX3 < W & AdjY3 >= 0 & AdjY3 < H & not cell(_, AdjX3, AdjY3) & not object(_, AdjX3, AdjY3) & dist(AdjX3, AdjY3, _)) {
                AdjX = AdjX3;
                AdjY = AdjY3;
            } else {
                // Try north
                AdjX4 = TargetX;
                AdjY4 = TargetY - 1;
                if (AdjX4 >= 0 & AdjX4 < W & AdjY4 >= 0 & AdjY4 < H & not cell(_, AdjX4, AdjY4) & not object(_, AdjX4, AdjY4) & dist(AdjX4, AdjY4, _)) {
                    AdjX = AdjX4;
                    AdjY = AdjY4;
                } else {
...
+!face_object(pot(N)) : player(X, Y, Dx, Dy, _) <-
    .findall(pos(Xp,Yp),cell(pot,Xp,Yp), Lpot);
    .nth(N,Lpot,pos(BestX,BestY));
    DirX = BestX - X;
    DirY = BestY - Y;
    if (DirX > 0) {
        !exec_action(east);
...
+!bfs_search(StartX, StartY, Object, Path) : true <-
    ...
    if(Object = pot(N)) {
        .findall(pos(X,Y),cell(pot,X,Y), Lpot);
        .nth(N,Lpot,pos(X,Y));
        !find_adjacent_position(X,Y,BestX,BestY);
    }
    else {
